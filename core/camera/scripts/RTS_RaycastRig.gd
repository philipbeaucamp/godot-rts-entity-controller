...
func _enter_tree():
	RTS_EventBus.set_camera_boundary.connect(on_set_camera_boundary)
	RTS_EventBus.set_camera_start_position.connect(on_camera_start_position)
...
func _process(delta):
	if !lock_mouse || !RTS_Controls.is_enabled:
		return
	...
	if Engine.time_scale != 0:
		...
		delta = RTS_Controls.time_utility.unscaled_delta

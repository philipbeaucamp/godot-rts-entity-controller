...
#Area3D that automatically implements a spatial hash grid for fast spatial queries.
#Entities with RTS_Entity component and space_hash enabled will be automatically
#added to the grid when entering the scene tree.

#A scene always requires a "main_grid" SpatialHashArea to work properly.
...
var origin: Vector3
var grid: RTS_SpatialHashFast
var entities: Dictionary[RTS_Entity,RTS_HashClient] = {} # keeps track of active entities
var clients: Dictionary[RTS_HashClient,RTS_Entity] = {}
var cell_size: float
var debug_color: Color

#Easy way to retrieve a main grid, set in _ready. Can be overwritten to achieve more complex behaviour
...
func _ready():
	...
	#subscription
	RTS_EventBus.entity_entered_tree.connect(on_entity_entered_tree)
	RTS_EventBus.entity_exiting_tree.connect(on_entity_exit_tree)
	...
	#set main_grid
	main_grid = self
	RTS_EventBus.grid_ready.emit(self)
...
func initialize_spatial_grid(_cell_size: float):
	...
	grid = RTS_SpatialHashFast.new(bounds,dimensions)
...
func get_selected_clients() -> Dictionary[RTS_HashClient,RTS_Entity]:
	var selectables : Array[RTS_Selectable] = RTS_Controls.selection.selection
	var selected_clients: Dictionary[RTS_HashClient,RTS_Entity] = {}

class_name MovementPaths extends Node3D

#Currently has a small bug that sometimes doesn't clean up chained attack/patrol paths properly

@export var selection: RTS_Selection
@export var colors: Dictionary[RTS_Movable.Type,Color] = {}
@export var pm: RTS_PoolManager

var paths: Dictionary[int,RTS_Path] = {} # target_group_id to  path
var paths_claims: Dictionary[int,int] #target_group_id to number of movables
var individual_paths: Dictionary[RTS_Entity,RTS_Path] # first path of unit
...
func create_path(start: Vector3, end: Vector3, type: RTS_Movable.Type, start_source: Node3D, end_source: Node3D,alpha_factor:float) -> RTS_Path:
	var path = path_pool.get_item(false) as RTS_Path

...
func _ready():
	...
	Controls.time_utility.unpaused.connect(on_unpaused)
	RTSEventBus.entity_exiting_tree.connect(on_entity_exiting_tree)
...
func process_input(input: Dictionary):
	...
	#HotkeySelection
	for i in range(9):
		var hotkey = RTSPlayerInput.hotkeys[i]
		if input[hotkey]:
...
func select_hotkey_group(key:int):
	...
	RTSEventBus.select_control_group.emit(key,hotkey_groups[key])

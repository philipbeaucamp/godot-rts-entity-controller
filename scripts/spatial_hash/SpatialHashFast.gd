...
var cells: Array #Array[Array[HashNode]] (rows/columns)
...
var clients: Dictionary[Client,bool] #only used for flood fill
...
func new_client(pos: Vector2, size: Vector2, group: int) -> Client:
	var c = Client.new()
	c.position = pos
	c.dimensions = size
	c.group = group 
...
func update_clients(_clients: Array[Client]):
	for c in _clients:
...
func find_near(pos: Vector2, _bounds: Vector2, exact: bool = true, group : int = -1) -> Array[Client]:
	var half = _bounds * 0.5
	var i1 = _get_cell_index(pos - half)
	var i2 = _get_cell_index(pos + half)
	var near_clients : Array[Client] = []
	var qid = query_ids
	query_ids += 1
	for x in range(i1.x, i2.x + 1):
		if x < 0 || x >= dimensions.x:
		...
		for y in range(i1.y, i2.y + 1):
			if y < 0 || y >= dimensions.y:
				continue
			var head : HashNode = cells[x][y]
			while head != null:
				var c : Client = head.client
				head = head.next
				...
				if c.query_id != qid:
					...
					if exact:
						...
						if pos.distance_squared_to(c.position) < l*l:
							near_clients.append(c as Client)
					else:
						near_clients.append(c as Client)
						
	return near_clients

func insert(c: Client):
	var half = c.dimensions * 0.5
	var i1 = _get_cell_index(c.position - half)
	var i2 = _get_cell_index(c.position + half)
	var nodes = []
	for x in range(i1.x, i2.x + 1):
		var x_nodes : Array[HashNode] = []
		if x < 0 || x >= dimensions.x:
			continue
		for y in range(i1.y, i2.y + 1):
			if y < 0 || y >= dimensions.y:
...
func remove(c: Client):
	...
	for x in range(i1.x, i2.x + 1):
		...
		for y in range(i1.y, i2.y + 1):
			...
			var xi = x - offset_x
			var yi = y - offset_y
			var node: HashNode = c._cells.nodes[xi][yi]
			if node.next:
...
func flood_fill_clusters(min_units_per_cluster : int = 1, group: int = -1, filter: Dictionary[Client,bool] = {}) -> Array:

...
var path_pool: ObjectPool
var waypoint_pool: ObjectPool
...
func add_paths(targets: Array[Target]):
...
func add_points(targets: Array[Target]):
	for target : Target in targets:
		if !target.display:
			continue
		#add claim
...
func remove_points(targets: Array[Target]):
	for target in targets:
		if !target.display:
			continue
		var id = target.group_id

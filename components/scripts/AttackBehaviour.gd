...
signal target_changed(attack: AttackBehaviour, old_target: Defense, new_target: Defense)
signal target_became_not_null(attack: AttackBehaviour, new_target: Defense)
signal target_became_null(attack: AttackBehaviour, old_target: Defense)
signal player_assigned_target_death(attack: AttackBehaviour, player_assigned_target: Defense)
signal current_target_death(attack: AttackBehaviour, target: Defense)
...
var current_target : Defense #can only be within scan range
var player_assigned_target: Defense #can be outside scan range
var weapon_target: Defense #guaranteed to be within weapon range
...
var aggressor: Defense #attacked by other defense

var defenses_in_weapon: Dictionary[Defense,bool] = {}
var defenses_in_scan: Array[Defense]
...
func set_active_weapon(weapon: Weapon):
	...
	if active_weapon:
		assert(!active_weapon.component_is_active, "Weapon already active when setting active weapon in AttackBehaviour")
...
func set_target(target: Defense):
	if current_target != target:
		var previous_target = current_target
		current_target = target
		# var target_name = str(target.entity.name) if target != null else "Null"
		# var prev_target_name = str(previous_target.entity.name) if previous_target != null else "Null"
		# Log.info_owner(self,"Setting target from " + str(prev_target_name) + " to " + str(target_name))
		if target != null:
...
func set_player_assigned_target(defense: Defense):
	...
	if defenses_in_scan.has(defense):
		set_target(defense)
	# #interrupt attack only if we're switching targets and can't attack immediately
	# elif previous != player_assigned_target:
	# 	do_interrupt_attack = true


#MOVEMENT
#only handles player_assigned_target changes, not state or current target (one exception) changes
func on_movable_next_target_changed(movable: Movable):
	var next: Target = movable.next
	var next_type :int = next.type if next else -1
	var previous_pat: Defense = player_assigned_target

	if next && !next.owner && (next_type == Movable.Type.ATTACK || next_type == Movable.Type.MOVEATTACK):
		var source = next.source
		if source != null && source is Entity && source.defense && source.defense != entity.defense:
			set_player_assigned_target(source.defense)

	can_auto_attack_current_target = !(next_type == Movable.Type.MOVE || next_type == Movable.Type.ATTACK || player_assigned_target)

	if (next_type == Movable.Type.MOVE || next_type == Movable.Type.ATTACK):
		aggressor = null

	match(next_type):
		Movable.Type.MOVE, Movable.Type.PATROL:
			do_interrupt_attack = true
	...
	# do_interrupt_attack = (
	# 	(next_type == Movable.Type.MOVE || next_type == Movable.Type.PATROL) 
	# 	|| (next_type == Movable.Type.MOVEATTACK && defenses_in_scan.is_empty()) #switching to move_attack from previously attacking ally unit, so we want to interruptt
	# 	#but we don't want to interrupt if we're currently attacking enemy unit
	# )
	if do_interrupt_attack:
		weapon_target = null

func on_final_target_reached(movable: Movable):
	can_auto_attack_current_target = true
...
# 2. Hold Position
# 3. Patrol
# 4. AttackBehaviour-Move
# 5. Follow Ally
# and NOT for
# 1. Move
# 2. AttackBehaviour Target
...
# 2. Which targets have the highest AttackBehaviour Target Priority (ATP) values?
# 3. Which targets allow me to use my primary weapon?
# 4. Only if I lost my previous target: Which target is the closest?
func try_auto_assign_target() -> Defense:
	if active_variant == null:
		return null

	#1. Threats
	var threats : Array[Defense] = []
	var attackable_threats: Array[Defense] = []
	...
	#2. ATP
	var highest_threats : Array[Defense] = []
	var highest_atp = -1
...
func remove_from_defenses_in_scan(defense: Defense):
	if defenses_in_scan.has(defense):
		defenses_in_scan.erase(defense)
...
func on_current_target_death(_entity: Entity):
...
func on_all_targets_cleared(_movable: Movable):
	stop()
...
func on_attacked_by(damage_dealer: DamageDealer):
	...
	if damage_dealer.publisher != null:
		...
		if  other_defense != null && other_defense.entity.faction != entity.faction:
			if entity.faction == Entity.Faction.NEUTRAL:
...
# func is_playing_attack_anim() -> bool:
# 	return attack_anim_timer != null && attack_anim_timer.time_left > 0

func is_attack_immobilized() -> bool:
	return immobilize_timer != null && immobilize_timer.time_left > 0

var movable_state_before_immobilization: Movable.State #todo recheck this

func on_tree_node_entered(node: StringName):
	if attack_nodes.has(node):
		var anim_tree: AnimationTreeComponent = entity.anim_tree
...
func start_immobilization_timer(immobile_duration: float):
	#assert(weapon_target,"Starting attack timer without weapon target")
	...
	if immobile_duration > 0 && entity.movable != null:
		...
		entity.movable.sm.set_state(Movable.State.IDLE)
...
func on_attack_anim_finished(anim_name: StringName= &""):
	if use_overlay_anim_for_attack_duration:
		...
		#assert(anim_name == "" || entity.anim_tree.overlay_anim_player.current_animation_length == entity.anim_tree.overlay_anim_player.current_animation_position)
	else:
		entity.anim_tree.animation_finished.disconnect(on_attack_anim_finished)
		#assert(anim_name == "" || entity.anim_tree.playback.get_current_play_position() == entity.anim_tree.playback.get_current_length())
...
# area_entered/exist only handle current_target changes, NOT state changes or player targets
func on_weapon_area_entered(area: Area3D):
	if area.component.entity == entity:
		return
	var defense = area.component as Defense
	...
	if !defenses_in_weapon.has(defense):
		...
		#not sure if best approach, but in case weapon_area_entered gets called before
		#scan_area_entered, manually call the latter to guarantee correct order
		if !defenses_in_scan.has(defense):
			# Log.info_owner(self,("Manually calling on scan area entered..."))
...
#swithc current target to other. this makes for better micro as the current
#target is not always followed
...
func on_scan_area_entered(area: Area3D):
	# Log.info_owner(self, area.owner.name + "ENTERED SCAN!!!!!!!!!!!!!!!!!!")
	# print_stack()
	if area.component.entity == entity:
		return
	var defense = area.component as Defense
	...
	if !defenses_in_scan.has(defense):
		# Log.info_owner(self,str(defense.entity.id) + " entered my scan area")
		defenses_in_scan.append(defense)
		#todo write comment
		# defense.entity.before_tree_exit.connect(on_before_tree_exit) 
		if defense == player_assigned_target:
			# Log.info_owner(self,"Player assigned target entered scan area")
...
func on_threat_changed(defense: Defense):
	if defense == current_target && !active_variant.can_attack(defense):
		var new_target = try_auto_assign_target()
		# settinig player_asigned target to null if new_target is not null ?
		set_target(new_target)

# func on_before_tree_exit(other: Entity):
# 	if defenses_in_scan.has(other):
# 		defenses_in_scan.erase(other)
# 	if defenses_in_weapon.has(other):
# 		defenses_in_weapon.erase(other)

...
var _pool: Array[RTS_ObjectPoolItem] = []
...
func get_item(set_active: bool) -> RTS_ObjectPoolItem:
	for i in _pool.size():
		var item = _pool[i]
		if !item.is_active:
			if set_active:
...
func retire_item(item: RTS_ObjectPoolItem) -> void:
	item.set_active(false)

func _create_instance() -> RTS_ObjectPoolItem:
	var instance : RTS_ObjectPoolItem = prefab.instantiate()

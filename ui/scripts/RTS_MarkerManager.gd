class_name RTS_MarkerManager extends Control

@export var markers: Dictionary[String,RTS_Marker] = {}
var prev_active_marker: RTS_Marker
var active_marker: RTS_Marker

func _ready():
	RTS_Controls.selection.hovered_pickable_set.connect(on_hovered_pickable_set)
	RTS_Controls.selection.hovered_pickable_empty.connect(on_hovered_pickable_empty)
	RTS_Controls.ability_manager.sm.state_changed.connect(on_abilities_state_changed)
	for key in markers:

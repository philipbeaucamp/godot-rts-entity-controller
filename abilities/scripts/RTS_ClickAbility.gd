class_name RTS_ClickAbility extends RTS_Ability
...
func cast():
	...
	if entity.movable && entity.movable.targets.size() > 0:
		entity.movable.on_next_target_reached(true)
	RTS_EventBus.click_ability_cast.emit(self)
...
func on_all_targets_cleared(_movable: RTS_Movable):
	soft_activate(false)
	terminated(true)
	RTS_EventBus.click_abilities_terminated.emit([self],true)
...
func _process(_delta):
	if can_cast():
		...
		soft_activate(false)
		terminated(false)
		RTS_EventBus.click_abilities_terminated.emit([self],false)

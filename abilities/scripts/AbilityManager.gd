		...
		if Input.is_action_just_pressed(key):
			...
			if abilities[0] is ClickAbility:
				...
				if !initiated_abilities.is_empty():
					...
					RTSEventBus.click_abilities_initiated.emit(initiated_abilities) #currently in cooldown abilities can be initiated further below
			else:
...
func clear_queued_click_abilities(cancelled: bool):
	...
	RTSEventBus.click_abilities_terminated.emit(initiated_abilities,cancelled)
	initiated_resource = null
	initiated_abilities.clear()
...
func refresh():
	on_selection_changed(Controls.selection.selection)

...
func _process(_delta):
	...
	if mouse_left_just_pressed:
		...
		if double_click_count == 1:
			double_click_timer = get_tree().create_timer(RTS_Controls.settings.double_click_time)
	...
	for i in range(9):
		...
		if pressed:
			...
			if timer != null && timer.time_left > 0 && last_pressed_hotkey == hotkey:
				...
				timer = get_tree().create_timer(RTS_Controls.settings.double_click_time)
	...
	#Controls (Camera)
	if RTS_Controls && RTS_Controls.is_enabled:
		RTS_Controls.process_input(input)
		
		#Abilities
		if RTS_Controls.ability_manager:
			if RTS_Controls.ability_manager.process_input(input):
				input["mouse_click_is_consumed"] = true

		#RTS_Selection
		if RTS_Controls.selection:
			RTS_Controls.selection.process_input(input)

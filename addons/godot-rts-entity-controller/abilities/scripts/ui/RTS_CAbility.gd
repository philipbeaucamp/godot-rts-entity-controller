class_name RTS_CAbility extends RTS_BlockingControl
...
@export var description: RTS_CAbilityDescription
...
func _ready():
	...
	RTS_Controls.ability_manager.abilities_activated.connect(on_abilities_activated)
	RTS_EventBus.click_ability_cast.connect(on_click_ability_cast)
	if resource is ClickAbilityResource:
		RTS_EventBus.click_abilities_initiated.connect(on_click_abilities_initiated)
		RTS_EventBus.click_abilities_terminated.connect(on_click_abilities_terminated)

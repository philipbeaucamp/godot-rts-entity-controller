class_name RTS_TacticalAI extends Node
...
#no reference anywhere and will immediately be freed
...
func update_clusters():
	...
	player_order_clusters_by_biggest = grid.flood_fill_clusters(1,Entity.Faction.PLAYER)
	...
	if !player_order_clusters_by_biggest.is_empty():
		...
		# 	for c in player_cluster_centers:
		# 		pass
				##DebugDraw3D.draw_cylinder_ab(c,c + Vector3.ONE,0.5,Color.ALICE_BLUE,1)
